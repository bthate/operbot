        ...
        if dirs:
            ...
            if d.count("-") == 2:
                ...
                if fls:
                    ...
                    if timed and "from" in timed and timed["from"] and op.utl.fntime(p) < timed["from"]:
                        continue
                    if timed and timed.to and op.utl.fntime(p) > timed.to:
                        continue
                    res.append(p)
    return sorted(res, key=op.utl.fntime)
