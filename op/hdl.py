...
class Event(op.Default):
    ...
    def parse(self):
        "parse an event"
        self.prs = op.Default()
        op.prs.parse(self.prs, self.otxt or self.txt)
...
class Handler(op.Object):
    ...
    def init(self, mns, name="ol"):
        "call init() of modules"
        thrs = []
        for mn in op.utl.spl(mns):
            try:
    ...
    def handler(self):
        ...
        while not self.stopped:
            ...
            if not e.orig:
                e.orig = repr(self)
            e.thrs.append(op.thr.launch(self.dispatch, e))
